...
def db_song_query(order, id):
    ...
    id, path, track_name, album, artist = queryResult['id'], queryResult[
        'path'], queryResult['track_name'], queryResult['album'], queryResult['artist']
    c.close()
...
def db_load_all_songs():
    ...
    for row in queryResults:
        track = AudioTrack(id=row[4], name=row[0], album=row[2],
                           artist=row[3], file_path=row[1].replace("\\", "\\\\"))
        tracks.append(track)
